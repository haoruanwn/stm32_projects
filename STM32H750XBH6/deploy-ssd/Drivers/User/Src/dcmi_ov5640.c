/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H743XIH6核心板 （型号：FK743M5-XIH6）+ OV5640模块（型号：OV5640M1-500W） 
...
// 写入固件之后，会有个初始化的过程，因此尝试读取100次状态，根据状态进行判断
	for(i=0;i<100;i++)	
	{
		...
		if( AF_Status == 0x7E)
		{
			printf("AF Firmware initialization in progress >>> \r\n");	
		}			
		if( AF_Status == 0x70)	// 释放马达，镜头回到初始（对焦为无穷远处）位置，意味着固件写入成功
		{
			printf("AF Firmware written successfully ! \r\n");
			return OV5640_Success;  
		}			
	}
// 尝试100次读取之后，还是没有读到0x70状态，说明固件没写入成功	
	printf("Autofocus firmware write failure !!! error >>> \r\n");	
	return OV5640_Error;	
}  
